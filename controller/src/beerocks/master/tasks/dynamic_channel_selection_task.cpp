...
void dynamic_channel_selection_task::work()
{
    ...
    case eState::TRIGGER_SCAN: {
        ...
        if (request == nullptr) {
            ...
            m_last_scan_error_code = beerocks::CHANNEL_SCAN_INTERNAL_FAILURE;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }

        int32_t dwell_time_msec = database.get_channel_scan_dwell_time_msec(m_radio_mac, m_is_single_scan);
        if (dwell_time_msec < 0) {
            LOG(ERROR) << "invalid dwell_time < 0";
            m_last_scan_error_code = beerocks::CHANNEL_SCAN_INVALID_PARAMS;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }
        ...
        if (curr_channel_pool.size() > beerocks::message::SUPPORTED_CHANNELS_LENGTH) {
            ...
            m_last_scan_error_code = beerocks::CHANNEL_SCAN_POOL_TOO_BIG;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }
    ...
    case eState::WAIT_FOR_SCAN_TRIGGERED: {
        ...
        m_last_scan_error_code = beerocks::CHANNEL_SCAN_TRIGGERED_EVENT_TIMEOUT;
    ...
    case eState::WAIT_FOR_RESULTS_READY: {
        ...
        m_last_scan_error_code = beerocks::CHANNEL_SCAN_RESULTS_READY_EVENT_TIMEOUT;
    ...
    case eState::WAIT_FOR_RESULTS_DUMP: {
        ...
        m_last_scan_error_code = beerocks::CHANNEL_SCAN_RESULTS_DUMP_EVENT_TIMEOUT;
    ...
    case eState::SCAN_DONE: {
        ...
        if (!m_is_single_scan) {
            auto interval = std::chrono::seconds(database.get_channel_scan_interval_sec(m_radio_mac));
            m_next_scan_timestamp_interval = m_last_scan_try_timestamp + interval;
        }

        m_last_scan_error_code = beerocks::CHANNEL_SCAN_NO_ERROR;
        database.set_channel_scan_results_status(m_radio_mac, m_last_scan_error_code, m_is_single_scan);
    ...
    if (m_radio_mac == event_radio_mac) {
        ...
        case eEvent::SCAN_RESULTS_DUMP: {
            ...
            if (FSM_IS_IN_STATE(WAIT_FOR_RESULTS_DUMP)) {
                ...
                } else if (!database.add_channel_scan_results(m_radio_mac,
        ...
        case eEvent::SCAN_ABORTED: {
            ...
            m_last_scan_error_code = beerocks::CHANNEL_SCAN_ABORTED_BY_DRIVER;
...
void dynamic_channel_selection_task::handle_events_timeout(std::multiset<int> pending_events)
{
    ...
    for (std::multiset<int>::iterator it = pending_events.begin(); it != pending_events.end();
        ...
        switch (m_fsm_state) {
        case eState::WAIT_FOR_SCAN_TRIGGERED: {
            m_last_scan_error_code = CHANNEL_SCAN_TRIGGERED_EVENT_TIMEOUT;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }
        case eState::WAIT_FOR_RESULTS_READY: {
            m_last_scan_error_code = CHANNEL_SCAN_RESULTS_READY_EVENT_TIMEOUT;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }
        case eState::WAIT_FOR_RESULTS_DUMP: {
            m_last_scan_error_code = CHANNEL_SCAN_RESULTS_DUMP_EVENT_TIMEOUT;
            FSM_MOVE_STATE(ABORT_SCAN);
            break;
        }
        case eState::SCAN_DONE:
