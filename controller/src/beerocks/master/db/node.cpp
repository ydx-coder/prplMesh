    ...
    : mac(mac_), capabilities(m_sta_24ghz_capabilities) // deafult value
{
    ...
    if ((type == beerocks::TYPE_CLIENT) || (type == beerocks::TYPE_IRE_BACKHAUL)) {
        ...
        hostap->continuous_scan_config = std::make_shared<radio::channel_scan_config>();
        hostap->continuous_scan_status = std::make_shared<radio::channel_scan_status>();
        hostap->single_scan_config     = std::make_shared<radio::channel_scan_config>();
        hostap->single_scan_status     = std::make_shared<radio::channel_scan_status>();
    }
