                                           ...
                                           const size_t max_buffer_size)
{
    ...
    nl_handler_cb_wrapper = [&](char *ifname, int event, int subevent, size_t len,
        ...
        if ((len == 0) || (data == NULL)) {
            LOG(ERROR) << __func__ << "len=0 and/or data is NULL ==> Abort!\n";
            return DWPAL_FAILURE;
        }
        if (event == NL80211_CMD_VENDOR) {
            if (len >= max_buffer_size) {
                LOG(ERROR) << __func__ << "NL size exceeds out_buffer size ==> Abort!\n";
                return DWPAL_FAILURE;
            }
            // copy get result from nl buffer, and sets the length copied to out_buffer_size
            *out_buffer_size = std::copy_n(data,len,out_buffer) - out_buffer;
        } else {
            LOG(ERROR) << __func__ << "non vendor event not handled ";
            return DWPAL_FAILURE;
        }
        return DWPAL_SUCCESS;
    };
