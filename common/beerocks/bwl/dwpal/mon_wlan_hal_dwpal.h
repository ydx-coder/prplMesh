...
private:
    bool dwpal_get_scan_params_fg(sScanCfgParams &params) {
        size_t out_len = 0;
        if (!dwpal_nl_cmd_get(m_radio_info.iface_name, LTQ_NL80211_VENDOR_SUBCMD_GET_SCAN_PARAMS,
                              m_nl_buffer, &out_len, NL_MAX_REPLY_BUFFSIZE)) {
            LOG(ERROR) << __func__ << " LTQ_NL80211_VENDOR_SUBCMD_GET_SCAN_PARAMS failed!";
            return false;
        }
        if (out_len != sizeof(params)) {
            LOG(ERROR) << __func__ << " returned size does not match sScanCfgParamsBG!";
            return false;
        }
        if (!memcpy_s(&params, sizeof(params), &m_nl_buffer[NL_ATTR_HDR], sizeof(params))) {
            LOG(ERROR) << __func__ << " memcpy_s failed!";
            return false;
        }
        return true;
    }
    bool dwpal_get_scan_params_bg(sScanCfgParamsBG &params) {
        size_t out_len = 0;
        if (!dwpal_nl_cmd_get(m_radio_info.iface_name, LTQ_NL80211_VENDOR_SUBCMD_GET_SCAN_PARAMS_BG,
                              m_nl_buffer, &out_len, NL_MAX_REPLY_BUFFSIZE)) {
