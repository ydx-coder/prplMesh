...
main() {
    ...
    [ "$DEPLOY" = "true" ] && {
        ...
        elif [ -n "$TARGET_IP" -a -n "$TARGET_USER" -a -n "$TARGET_PASS" ]; then

            echo "scp ${BUILD_DIR}/prplmesh.tar.gz to ${TARGET_USER}@${TARGET_IP}"
            run sshpass -p ${TARGET_PASS} scp ${SSH_OPTIONS} ${BUILD_DIR}/prplmesh.tar.gz ${TARGET_USER}@${TARGET_IP}:/tmp

            echo "remote md5sum"
            run sshpass -p ${TARGET_PASS} ssh ${SSH_OPTIONS} ${TARGET_USER}@${TARGET_IP} md5sum /tmp/prplmesh.tar.gz

            echo "backaup /opt/prplmesh/beerocks_agent.conf on target"
            run sshpass -p ${TARGET_PASS} ssh ${SSH_OPTIONS} ${TARGET_USER}@${TARGET_IP} cp /opt/prplmesh/config/beerocks_agent.conf /tmp/

            echo "untar /tmp/prplmesh.tar.gz on target"
            run sshpass -p ${TARGET_PASS} ssh ${SSH_OPTIONS} ${TARGET_USER}@${TARGET_IP} tar -C / -xzvf /tmp/prplmesh.tar.gz

            echo "restore /opt/prplmesh/beerocks_agent.conf on target"
            run sshpass -p ${TARGET_PASS} ssh ${SSH_OPTIONS} ${TARGET_USER}@${TARGET_IP} cp /tmp/beerocks_agent.conf /opt/prplmesh/config/
        fi
    }
